...
(`rate(5 minutes)`) and does six things, five of them using the PAT: deregisters GitHub
...
has stopped making progress**; terminates stray `ami-builder-temp` instances older than 2
hours (pure EC2, no PAT); reaps launches still `pending` after 15 minutes; and counts
