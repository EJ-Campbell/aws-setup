...
# ---------------------------------------------------------------------------------
# Hop targets, keyed by their Elastic IP -- not their private IP.
#
...
# out. Elastic IPs are re-associated by terraform on every replacement
# (aws_eip_association), so baking them in here means the hop config is correct as of the
# next `terraform apply` -- no boot-time AWS API call, nothing that can go stale between
# boots waiting for a box to reboot on its own.
# ---------------------------------------------------------------------------------
locals {
  dev_hop_targets = {
    for k, v in {
...
if [ -n "$HOPJSON" ]; then
  ...
  #
  # This content is STATIC -- rendered by terraform from the Elastic IPs at apply time, not
  # looked up from the AWS API at boot. There is nothing here that goes stale between boots;
  # it only changes when terraform re-applies, at which point the updated setup script needs
  # to be re-fetched and re-run on each box (same as any other change to this file).
  cat > /home/ubuntu/.ssh/config.d-devhop <<'DEVHOPCFG'
# generated by terraform (dev-hop-key.tf) from stable Elastic IPs -- do not edit by hand
